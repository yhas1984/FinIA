# Ficha de Play Store — FinAI (es-ES)

> Estado: texto final optimizado con skill de ASO (2026-08-16).
> Keywords objetivo cubiertas: control de gastos · gestor de gastos · gastos e ingresos · finanzas personales · facturas · presupuesto · ahorro · gastos diarios · escanear.

## Nombre (23/30)

```
FinAI: Control de Gastos
```

## Descripción breve (80/80)

```
Controla gastos e ingresos, escanea facturas, lleva tu presupuesto y ahorra con IA.
```

## Descripción completa (3.313/4.000)

```
FinAI: Control de Gastos — tu gestor de gastos e ingresos con inteligencia artificial. Olvídate de las hojas de cálculo: registra tus gastos diarios, escanea facturas y entiende tus finanzas personales hablando con la app, como con un asesor financiero. Pensada para autónomos y pymes que necesitan controlar sus gastos e ingresos del día a día y mantener su presupuesto bajo control.

📷 ESCANEA FACTURAS CON LA CÁMARA
Toma una foto de cualquier factura o ticket para escanear sus datos al instante. La IA extrae automáticamente proveedor, fecha, número de factura, total, base imponible, IVA, IRPF, NIF y productos con precios. Soporta gastos e ingresos: nóminas, facturas de venta y tickets. Sin escribir ni un solo dato a mano.

🤖 ASISTENTE DE IA
Integra Gemini 3.6 Flash para controlar tus gastos de forma natural. Di "gasté 20€ en café" o "cobré 1500€ de nómina" y se registra al instante. Pregunta "¿cuánto gasté este mes?", "¿cuál es mi balance?" o "¿cuánto gasté en supermercado?" y obtén la respuesta al momento. La IA recuerda el contexto de la conversación y sugiere categorías y subcategorías automáticamente.

🎙️ COMANDOS POR VOZ
Dicta tus transacciones mientras conduces o cocinas: la voz se convierte en texto y la IA la procesa al instante.

📊 DASHBOARD FINANCIERO
Balance del mes, ingresos, gastos y actividad semanal de un vistazo. Estadísticas interactivas con porcentajes, calendario financiero mensual y widgets reordenables para ver solo lo que te importa. Entiende en qué gastas, ajusta tu presupuesto… y ahorra más.

📋 GESTIÓN COMPLETA
Facturas con proveedor, número de factura, fecha, IVA, IRPF, NIF, base imponible e imagen. Ingresos con concepto, devengado, neto, moneda y fuente. Productos con cantidad, precio unitario y subtotal. Categorías y subcategorías personalizables, con configuración fiscal por país: España, México, Argentina, Colombia, Chile, Perú y más.

☁️ EXPORTACIÓN A GOOGLE SHEETS
Exporta tus datos a un Google Sheet con 4 hojas: Gastos, Ingresos, Productos y Resumen con fórmulas automáticas. Una vez vinculado, cada alta, edición o borrado se sincroniza automáticamente.

💾 BACKUP EN GOOGLE DRIVE
Copia de seguridad cifrada (AES-256) de tu base de datos en tu Drive personal, con versiones anteriores y restauración. También exporta a CSV o PDF para compartir por correo o WhatsApp.

🔒 PRIVACIDAD Y SEGURIDAD
Tus datos financieros se almacenan exclusivamente en tu dispositivo. Sin publicidad ni rastreadores. Google Sheets y Drive requieren tu autorización OAuth explícita y solo acceden a los archivos que crea la app. Sin registro ni cuenta obligatoria.

⭐ PREMIUM — compra única
• Exportación y sincronización con Google Sheets.
• Backup automático en Google Drive.
• Chat IA con memoria extendida (10 turnos).

REQUISITOS
Android 8.0+. Internet necesario para el asistente IA (los mensajes se envían a la API de Gemini). API key gratuita de Gemini configurable en Ajustes. Cámara y micrófono opcionales.

Descarga FinAI gratis y toma el control de tu dinero: registra, escanea facturas y ahorra cada mes.
```

## Release notes v1.6.0

```
🔹 Subcategorías opcionales para gastos e ingresos, con sugerencias y categorías personalizadas.
🔹 Nuevo Dashboard con selector mensual, estadísticas interactivas, porcentajes e importes.
🔹 Drill-down de categoría a subcategoría y movimientos editables.
🔹 Widgets del Dashboard reordenables y ocultables según tus preferencias.
🔹 Botones flotantes movibles en Dashboard, Gastos e Ingresos, con posiciones independientes.
🔹 Las preferencias del Dashboard y de los botones flotantes se incluyen en los backups.
```
